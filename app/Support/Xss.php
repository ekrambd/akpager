...
class Xss
{
    ...
    {
        $cachePath = $this->config->get('xss.cachePath');

        if ($cachePath) {
            $this->files->makeDirectory(
                $cachePath,
                $this->config->get('xss.cacheFileMode', 0755),
                true,
                true
            );
        }
    ...
    protected function getConfig($config = null)
    {
        ...
        if (! $this->config->get('xss.finalize')) {
            $configObject->autoFinalize = false;
        }

        // Set default config
        $defaultConfig = [];
        $defaultConfig['Core.Encoding'] = $this->config->get('xss.encoding');
        $defaultConfig['Cache.SerializerPath'] = $this->config->get('xss.cachePath');
        $defaultConfig['Cache.SerializerPermissions'] = $this->config->get('xss.cacheFileMode', 0755);

        if (! $config) {
            $config = $this->config->get('xss.settings.default');
        } elseif (is_string($config)) {
            $config = $this->config->get('xss.settings.' . $config);
        }
        ...
        if ($definitionConfig = $this->config->get('xss.settings.custom_definition')) {
            $this->addCustomDefinition($definitionConfig, $configObject);
        }

        // Load custom elements if set
        if ($elements = $this->config->get('xss.settings.custom_elements')) {
            if ($def = $configObject->maybeGetRawHTMLDefinition()) {
        ...
        if ($attributes = $this->config->get('xss.settings.custom_attributes')) {
            if ($def = $configObject->maybeGetRawHTMLDefinition()) {
    ...
    public function clean($dirty, $config = null, \Closure $postCreateConfigHook = null)
    {
        ...
        $ignoreNonStrings = $this->config->get('xss.ignoreNonStrings', false);

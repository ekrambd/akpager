...
class Theme
{
   /**
    ...
    {
        if (File::isDirectory($this->path) && $this->enable == false) {
            // delete files
    ...
    public function languages()
    {
        ...
        // fixed error
        if (!is_dir($dir = base_path("{$this->path}/lang/{$langs['iso_639_1']}"))) {
        ...
        // dd(
        //     glob(base_path("{$this->path}/lang/*"),  GLOB_ONLYDIR | GLOB_ERR)
        // );

        foreach (glob(base_path("{$this->path}/lang/*"),  GLOB_ONLYDIR | GLOB_ERR) as $path) {
            $code          = basename($path);
    ...
    {
        $paths = [];
        foreach (glob(base_path("{$this->path}/views/layouts/*.blade.php")) as $path) {

            $key  = str_replace('.blade.php', '', basename($path));

            $value = 'theme::layouts.' . $key;
            $paths[$key]  = $value;
        }
